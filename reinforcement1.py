...
def build_q_table(n_states, actions):
    ...
    table = pd.read_csv("q_table_reinforce_4timesshuffle_2.csv")
...
def update_env(S, episode, step_counter):
    ...
    if S == solved:
        ...
        print('\r{}'.format(b.updatestate()), end='')
...
def rl():
    ...
    for i in range(1485):
        print("state of index : " + str(i+7613))
...
def test(S):
    if S != solved:
        state_actions = q_table.iloc[training.index(S), :]
...
#q_table.to_csv('q_table_reinforce_4timesshuffle_2.csv', index=False, header=True)
#print('done')
'''
truth = []
for sample in training:
    try:
        ...
        truth.append(training.index(sample))
'''
